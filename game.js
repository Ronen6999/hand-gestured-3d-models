...
export var Game = /*#__PURE__*/ function() {
    ...
    function Game(renderDiv) {
        ...
        this.availableModels = [
            { name: 'Girl', path: 'assets/girl.glb', scale: 80 },
            { name: 'Desert Eagle', path: 'assets/Desert_eagle.glb', scale: 80 },
            { name: 'Assault Rifle', path: 'assets/assault_rifle.gltf', scale: 80 },
            { name: 'M4A1', path: 'assets/M4A1.glb', scale: 80 }
        ];
        this.currentModelIndex = 0;
        this.modelSwitcherContainer = null;
        this.modelSwitcherButtons = {};
    ...
    _create_class(Game, [
        ...
        },
        {
            ...
            value: function _setupDOM() {
                ...
                this.speechBubble.style.padding = '15px 30px';
                this.speechBubble.style.background = 'linear-gradient(135deg, rgba(236, 72, 153, 0.95) 0%, rgba(239, 68, 68, 0.95) 100%)';
                this.speechBubble.style.border = 'none';
                this.speechBubble.style.borderRadius = '16px';
                this.speechBubble.style.boxShadow = '0 4px 20px rgba(236, 72, 153, 0.4), 0 2px 8px rgba(0, 0, 0, 0.2)';
                this.speechBubble.style.color = 'white';
                this.speechBubble.style.fontFamily = '"Segoe UI", "Roboto", "Arial", sans-serif';
                this.speechBubble.style.fontSize = 'clamp(16px, 3vw, 22px)';
                this.speechBubble.style.fontWeight = '600';
                this.speechBubble.style.maxWidth = '80%';
                this.speechBubble.style.textAlign = 'center';
                this.speechBubble.style.zIndex = '25';
                this.speechBubble.style.opacity = '0';
                this.speechBubble.style.transition = 'all 0.4s cubic-bezier(0.4, 0, 0.2, 1)';
                ...
                this._setupModelSwitcher(); // Add model switcher UI
            }
        },
        {
            key: "_setupThree",
            value: function _setupThree() {
                ...
                for(var i = 0; i < 2; i++){
                    ...
                    this.hands.push({
                        ...
                        wasPinching: false // For button interaction debouncing
                    });
                }
                    ...
                    return _ts_generator(this, function(_state) {
                        switch(_state.label){
                            case 0:
                                ...
                                // Load initial model from availableModels array
                                return [
                                    4,
                                    _this._loadModelFromConfig(_this.availableModels[_this.currentModelIndex])
                                ];
                            case 1:
                                _state.sent();
                ...
                if (videoTime > this.lastVideoTime) {
                    ...
                    try {
                        ...
                        for(var i = 0; i < this.hands.length; i++)_this1 = this, _loop(i);
                         // End of hand loop
                        // Check for button interactions with pinch gestures
                        for(var i = 0; i < this.hands.length; i++){
                            var hand = this.hands[i];
                            if (hand.isPinching && hand.pinchPointScreen) {
                                // Check interaction mode buttons (drag, rotate, scale, animate)
                                ['drag', 'rotate', 'scale', 'animate'].forEach(function(mode) {
                                    var button = _this.interactionModeButtons[mode];
                                    if (button && _this._checkButtonHover(hand.pinchPointScreen.x, hand.pinchPointScreen.y, button)) {
                                        // Debounce: only trigger if this is a new pinch
                                        if (!hand.wasPinching) {
                                            console.log("Pinch detected over interaction mode button: ".concat(mode));
                                            _this._setInteractionMode(mode);
                                        }
                                    }
                                });
                                // Check model switcher buttons
                                this.availableModels.forEach(function(model, modelIndex) {
                                    var button = _this.modelSwitcherButtons[model.name];
                                    if (button && _this._checkButtonHover(hand.pinchPointScreen.x, hand.pinchPointScreen.y, button)) {
                                        // Debounce: only trigger if this is a new pinch
                                        if (!hand.wasPinching) {
                                            console.log("Pinch detected over model switcher button: ".concat(model.name));
                                            _this._switchModel(modelIndex);
                                        }
                                    }
                                });
                            }
                            hand.wasPinching = hand.isPinching; // Track previous pinch state for debouncing
                        }
        ...
        {
            key: "_checkButtonHover",
            value: function _checkButtonHover(pinchX, pinchY, button) {
                if (!button) return false;
                var rect = button.getBoundingClientRect();
                // Convert pinch coordinates from center-origin to screen coordinates
                var screenX = pinchX + this.renderDiv.clientWidth / 2;
                var screenY = -pinchY + this.renderDiv.clientHeight / 2;
            ...
            value: function _updateInteractionModeButtonStyles() {
                ...
                for(var modeKey in this.interactionModeButtons){
                    ...
                    if (modeKey === this.interactionMode) {
                        // Active button - vibrant gradient
                        if (modeConfig) {
                            var rgb = modeConfig.hand;
                            var r = Math.floor(rgb.r * 255);
                            var g = Math.floor(rgb.g * 255);
                            var b = Math.floor(rgb.b * 255);
                            button.style.background = "linear-gradient(135deg, rgba(".concat(r, ", ").concat(g, ", ").concat(b, ", 0.95) 0%, rgba(").concat(Math.max(0, r - 30), ", ").concat(Math.max(0, g - 30), ", ").concat(Math.max(0, b - 30), ", 0.95) 100%)");
                            button.style.boxShadow = "0 4px 15px rgba(".concat(r, ", ").concat(g, ", ").concat(b, ", 0.4), 0 2px 5px rgba(0, 0, 0, 0.2)");
                        } else {
                            button.style.background = 'linear-gradient(135deg, rgba(108, 117, 125, 0.95) 0%, rgba(73, 80, 87, 0.95) 100%)';
                            button.style.boxShadow = '0 4px 15px rgba(108, 117, 125, 0.4), 0 2px 5px rgba(0, 0, 0, 0.2)';
                        }
                        button.style.color = 'white';
                        button.style.transform = 'scale(1.05)';
                    } else {
                        // Inactive button - subtle glass effect
        ...
        {
            key: "_setupModelSwitcher",
            value: function _setupModelSwitcher() {
                var _this = this;
                // Model Switcher Container
                this.modelSwitcherContainer = document.createElement('div');
                this.modelSwitcherContainer.id = 'model-switcher-container';
                this.modelSwitcherContainer.style.position = 'absolute';
                this.modelSwitcherContainer.style.bottom = '10px';
                this.modelSwitcherContainer.style.left = '10px';
                this.modelSwitcherContainer.style.zIndex = '30';
                this.modelSwitcherContainer.style.display = 'flex';
                this.modelSwitcherContainer.style.flexDirection = 'row';
                this.modelSwitcherContainer.style.gap = '8px';
                this.renderDiv.appendChild(this.modelSwitcherContainer);
                // Create model switcher buttons
                this.availableModels.forEach(function(model, index) {
                    var button = document.createElement('button');
                    button.innerText = model.name;
                    button.id = "model-switch-".concat(model.name.toLowerCase());
                    button.style.padding = '12px 24px';
                    ...
                    button.style.backdropFilter = 'blur(10px)';
                    button.addEventListener('click', function() {
                        return _this._switchModel(index);
                    });
                    _this.modelSwitcherContainer.appendChild(button);
                    _this.modelSwitcherButtons[model.name] = button;
                });
                this._updateModelSwitcherStyles();
            }
        },
        {
            key: "_updateModelSwitcherStyles",
            value: function _updateModelSwitcherStyles() {
                var _this = this;
                this.availableModels.forEach(function(model, index) {
                    var button = _this.modelSwitcherButtons[model.name];
                    if (button) {
                        if (index === _this.currentModelIndex) {
                            // Active model - green gradient
                            button.style.background = 'linear-gradient(135deg, rgba(16, 185, 129, 0.95) 0%, rgba(5, 150, 105, 0.95) 100%)';
                            button.style.color = 'white';
                            button.style.boxShadow = '0 4px 15px rgba(16, 185, 129, 0.4), 0 2px 5px rgba(0, 0, 0, 0.2)';
                            button.style.transform = 'scale(1.05)';
                        } else {
                            // Inactive model - glass effect
                            button.style.background = 'rgba(255, 255, 255, 0.15)';
                            button.style.color = 'rgba(255, 255, 255, 0.9)';
                            button.style.boxShadow = '0 2px 8px rgba(0, 0, 0, 0.1)';
                            button.style.transform = 'scale(1)';
        ...
        {
            key: "_switchModel",
            value: function _switchModel(modelIndex) {
                var _this = this;
                return _async_to_generator(function() {
                    var modelConfig;
                    return _ts_generator(this, function(_state) {
                        switch(_state.label){
                            case 0:
                                if (modelIndex === _this.currentModelIndex) {
                                    console.log("Model already loaded.");
                                    return [
                                        2
                                    ];
                                }
                                if (modelIndex < 0 || modelIndex >= _this.availableModels.length) {
                                    console.warn("Invalid model index: ".concat(modelIndex));
                                    return [
                                        2
                                    ];
                                }
                                modelConfig = _this.availableModels[modelIndex];
                                console.log("Switching to model: ".concat(modelConfig.name));
                                _this.currentModelIndex = modelIndex;
                                _this._updateModelSwitcherStyles();
                                return [
                                    4,
                                    _this._loadModelFromConfig(modelConfig)
                                ];
                            case 1:
                                _state.sent();
                                return [
        ...
        {
            key: "_loadModelFromConfig",
            value: function _loadModelFromConfig(modelConfig) {
                var _this = this;
                return _async_to_generator(function() {
                    var gltfLoader;
                    return _ts_generator(this, function(_state) {
                            ...
                            case 0:
                                ...
                                return [
                                    ...
                                    new Promise(function(resolve, reject) {
                                        gltfLoader.load(modelConfig.path, function(gltf) {
                                            // Remove old model
                                            if (_this.pandaModel) {
                                            ...
                                            if (_this.animationClips && _this.animationClips.length) {
                                                ...
                                                console.log("".concat(modelConfig.name, " model has no embedded animations."));
                                            }
                                            // Scale and position
                                            var scale = modelConfig.scale || 80;
                                            ...
                                            console.log("".concat(modelConfig.name, " model loaded and added to scene."));
                                            resolve();
                                        }, undefined, function(error) {
                                            console.error("Error loading model ".concat(modelConfig.name, ":"), error);
                                            reject(error);
                                        });
                                    })
                                ];
